...
def validation(args):
    ...
    # custom method for loading last checkpoint
    ckpt = torch.load(os.path.join(args.checkpoint_dir, "morphized_model.ckpt"), map_location='cpu')
    print("========================================================================\n"
    ...
    with torch.no_grad():
        for eval_imgs in validate_dataloader:
            ...
            stream_list, shape_list = coder.compress(eval_imgs)

            eval_size = sum(len(stream) for stream in stream_list)
            eval_rate = eval_size * 8 / (eval_imgs.size(2) * eval_imgs.size(3))

            _, eval_est_rate = coder(eval_imgs)

            eval_imgs_tilde = coder.decompress(stream_list, shape_list)

            eval_aux_loss = coder.entropy_bottleneck.aux_loss()

            eval_imgs *= 255
            eval_imgs = eval_imgs.clamp(0, 255).round()
            eval_imgs_tilde *= 255
            eval_imgs_tilde = eval_imgs_tilde.clamp(0, 255).round()

            eval_psnr = PSNR(eval_imgs, eval_imgs_tilde, data_range=255.)
            eval_msssim = ms_ssim(eval_imgs, eval_imgs_tilde, size_average=True,
                                  nonnegative_ssim=True).item()

            eval_psnr_list.append(eval_psnr)
            eval_msssim_list.append(eval_msssim)
            eval_rate_list.append(eval_rate)
            eval_est_rate_list.append(eval_est_rate.item())
            eval_aux_loss_list.append(eval_aux_loss.item())

            print("PSNR: {:.4f}, MS-SSIM: {:.4f}, est.rate: {:.4f} rate: {:.4f}, aux: {:.4f}".format(
                eval_psnr,
                eval_msssim,
                eval_est_rate.item(),
                eval_rate,
