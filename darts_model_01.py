...
def train(args):
    ...
    for epoch in range(start_epoch, args.max_epochs):
        ...
        for idx, imgs in enumerate(pbar):
            ...
            # update alphas with validation

            randsamplr = torch.utils.data.RandomSampler(train_dataset, True, 10)
            alpha_train_dataloader = data.DataLoader(train_dataset, 1, False, randsamplr, num_workers=16)

            coder.module.alphas['alpha_conv'].requires_grad = True
            coder.module.alphas['alpha_deconv'].requires_grad = True
            coder.module.alphas['alpha_acti'].requires_grad = True
            
            optim3.zero_grad()
            for eval_imgs in alpha_train_dataloader:
                
                eval_imgs = eval_imgs.to(DEVICE)

                eval_imgs_tilde, eval_rate = coder(eval_imgs)

                eval_distortion = torch.mean(torch.pow(eval_imgs - eval_imgs_tilde, 2), dim=[1, 2, 3])
                eval_distortion *= (255 ** 2)
                eval_distortion = torch.clamp_max(eval_distortion, 1e8)

                eval_rate = torch.squeeze(eval_rate)
                eval_distortion = torch.squeeze(eval_distortion)

                val_loss = torch.mean(args.lmda * eval_distortion + eval_rate)
                val_loss.backward()
                optim3.step()
            ...
            writer_record['alpha_loss'].append(val_loss.cpu().item())
